...
public class Serie {
	...
	public void desazonalizar(){
		...
		for(SerieYear sy : timedSerie){
			...
			for(int month = 0; month < 12; month++){
				sy.riverFlow.set(month,
						(sy.riverFlow.get(month)*standardDeviation[month] + averagePerMonth.get(month))
						);
			}
		}
		...
		for (int i = fullSerie.size() - forecastSet.size(), j = 0; i + j < fullSerie.size(); j++){
			int month = ((i + j) % 11);
			forecastSet.set(j, forecastSet.get(j)*standardDeviation[month] + averagePerMonth.get(month));
		}
